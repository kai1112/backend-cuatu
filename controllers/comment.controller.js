...
const create = async (req, res) => {
  try {
    ...
    if (!req.body.ProductID)
      return res.json({ status: 404, message: "ProductID is required!!!" });
    ...
    if (!newComment)
      return res.json({ status: 404, message: "Create Comment failed!" });
    return res.json({
      status: 200,
...
const update = async (req, res) => {
  try {
    ...
    if (!Comment)
      return res.json({ status: 202, message: "Comment is not found!!!" });
    if (req.body.content) Comment.content = req.body.content;
    let updateComment = await Comment.findOneAndUpdate(Comment);
    if (!updateComment)
      return res.json({ status: 202, message: "Update Comment is failed!!!" });
    return res.json({
      status: 200,
      message: "Update Comment is successfully",
      data: Comment,
    });
  } catch (err) {
    createLogger.error(err);
...
const remove = async (req, res) => {
  try {
    ...
    let updateComment = await Comment.findOneAndUpdate(Comment);
    if (!updateComment)
      return res.json({ status: 202, message: "Remove Comment is failed!!!" });
    return res.json({
      status: 200,
      message: "Remove Comment is successfully",
      data: Comment,
    });
  } catch (err) {
    createLogger.error(err);
